...
public class BookingComponent {
	...
	public long book(BookingRecord record) {
		...
		bookingDetails.put("NEW_INVENTORY", inventory.getAvailable()); // added by Praveen
	...
	public Optional<BookingRecord> getBooking(long id) {
		System.out.println("----- >> Trying "+bookingRepository.existsById(id));
		//System.out.println("----- >> "+bookingRepository.getOne(id).getFlightNumber());
	...
	public void updateStatus(String status, long bookingId) {
		...
		if (record != null) { // added by Praveen s.
			record.setStatus(status);
			logger.info("Updating status = " + status + " of bookingId = " + bookingId); // Added by Praveen s.
			bookingRepository.save(record);// Added by Praveen s
		}
	}
}
